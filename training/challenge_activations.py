...
for model_name in model_list:
    for act in [None, 'relu']:
        for seed in range(5):
            model_spec = f'{model_name}-{seed}-noact' if not act else f'{model_name}-{seed}-relu'
            wandb.init(project="synthetic-benchmark", reinit=True, 
                       name=model_spec)
            wandb.config.update({'model_type' : model_name, 'seed' : seed,
                                'batch_size' : batch_size, 'activation' : act,
            ...
            model = getattr(models, model_name)(hidden_act=act, classes=classes)
